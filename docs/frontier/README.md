...
Rewards use the installed ItemReward implementation, which splits delivery according to `ItemStack.getMaxStackSize()`. Every new reward entry is at most 64 items; known smaller limits are split explicitly (for example Vista cassettes). This native delivery behavior was inspected, but inventory overflow and client claim behavior remain runtime checks. No claim is made that a resource-ID check proves every reward's maximum stack size.
