...
import argparse, copy, hashlib, io, itertools, json, math, re, zipfile
...
SHAPES = {"circle", "square", "diamond", "hexagon", "octagon", "pentagon", "gear", "heart", "rsquare", "none"}
LINK_ID_RE = re.compile(r"^[0-9A-Fa-f]{16}$")
...
def normalize_quest(q: dict[str, Any]) -> dict[str, Any]:
    # A renderer pass must never alter the manifest object supplied by its
    # caller.  In particular, dependencies and icon compounds are nested
    # values and a shallow copy is not sufficient here.
    q = copy.deepcopy(q); q.setdefault("id", ""); q.setdefault("title", q.get("id", "Untitled")); q.setdefault("x", 0); q.setdefault("y", 0)
    q.setdefault("size", 1.0); q.setdefault("shape", ""); q.setdefault("dependencies", []); q.setdefault("optional", False)
    q["dependencies"] = list(q.get("dependencies") or [])
    q.setdefault("hide_dependency_lines", None); q.setdefault("icon", {})
    if isinstance(q["icon"], dict): q["icon"] = q["icon"].get("id", "")
    return q


def _link_id(seed: str, used: set[str]) -> str:
    """Return a deterministic native-looking ID that is unique in the book."""
    candidate = hashlib.sha256(seed.encode("utf-8")).hexdigest()[:16].upper()
    if candidate not in used:
        return candidate
    # Hashing a counter is preferable to appending a non-native suffix: FTB
    # quest and link IDs are conventionally exactly sixteen hexadecimal chars.
    for counter in itertools.count(1):
        candidate = hashlib.sha256(f"{seed}:{counter}".encode("utf-8")).hexdigest()[:16].upper()
        if candidate not in used:
            return candidate


def _normalize_link(link: Any, chapter: dict[str, Any], index: int, used: set[str], errors: list[str]) -> dict[str, Any] | None:
    if not isinstance(link, dict):
        errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {index} is not an object")
        return None
    result = copy.deepcopy(link)
    raw_id = str(result.get("id", ""))
    if not LINK_ID_RE.fullmatch(raw_id):
        generated = _link_id(f"quest-link:{chapter.get('filename', '')}:{index}:{result.get('linked_quest', '')}:{raw_id}", used)
        if raw_id:
            errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link id {raw_id!r} is not a 16-hex ID; using {generated}")
        else:
            errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {index} has no ID; using {generated}")
        raw_id = generated
    raw_id = raw_id.upper()
    if raw_id in used:
        generated = _link_id(f"quest-link:{chapter.get('filename', '')}:{index}:{result.get('linked_quest', '')}:{raw_id}", used)
        errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: duplicate quest link ID {raw_id}; using {generated}")
        raw_id = generated
    used.add(raw_id)
    result["id"] = raw_id
    target = result.get("linked_quest", "")
    result["linked_quest"] = str(target) if target is not None else ""
    for key, default in (("x", 0.0), ("y", 0.0), ("size", 0.8)):
        try:
            result[key] = float(result.get(key, default))
        except (TypeError, ValueError):
            errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {raw_id} has invalid {key!r}; using {default}")
            result[key] = float(default)
    shape = str(result.get("shape", "diamond") or "diamond").lower()
    if shape not in SHAPES:
        errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {raw_id} has unsupported shape {shape!r}; rendering as diamond")
        shape = "diamond"
    result["shape"] = shape
    return result


def _link_visuals(chapters: list[dict[str, Any]]) -> list[str]:
    """Attach private visual projections while preserving actual chapter data."""
    global_quests: dict[str, tuple[dict[str, Any], dict[str, Any]]] = {}
    errors: list[str] = []
    used: set[str] = set()
    for chapter in chapters:
        for quest in chapter.get("quests", []):
            qid = str(quest.get("id", ""))
            if qid in global_quests:
                errors.append(f"duplicate quest ID {qid!r}; link target resolution uses the first occurrence")
                continue
            global_quests[qid] = (quest, chapter)
            if LINK_ID_RE.fullmatch(qid):
                used.add(qid.upper())
    # Native IDs are case-sensitive in practice, but all emitted IDs are
    # uppercase.  Resolve exact IDs first and retain a case-insensitive
    # fallback for hand-authored manifests.
    folded = {qid.lower(): value for qid, value in global_quests.items()}
    for chapter in chapters:
        chapter_errors: list[str] = []
        # Strip private projections from a prior normalization pass so a
        # second pass is byte-for-byte/idempotent.
        chapter.pop("_render_quests", None)
        chapter.pop("_link_target_map", None)
        links: list[dict[str, Any]] = []
        raw_links = chapter.get("quest_links", []) or []
        if not isinstance(raw_links, list):
            chapter_errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest_links must be a list")
            raw_links = []
        for index, raw_link in enumerate(raw_links):
            link = _normalize_link(raw_link, chapter, index, used, chapter_errors)
            if link is not None:
                links.append(link)
        chapter["quest_links"] = links
        actual = [normalize_quest(q) for q in chapter.get("quests", []) if isinstance(q, dict)]
        actual_ids = {str(q.get("id", "")) for q in actual}
        target_to_link: dict[str, str] = {}
        for link in links:
            target = link["linked_quest"]
            if not target:
                chapter_errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {link['id']} has no linked_quest target")
                continue
            if target not in global_quests and target.lower() not in folded:
                chapter_errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: quest link {link['id']} target {target!r} is missing")
                continue
            # A dependency can only point to one visual endpoint.  Keep the
            # first deterministic link and report subsequent duplicates.
            if target in target_to_link:
                chapter_errors.append(f"{chapter.get('filename', chapter.get('title', 'chapter'))}: multiple quest links target {target!r}; dependencies use {target_to_link[target]}")
            else:
                target_to_link[target] = link["id"]
                target_to_link.setdefault(target.lower(), link["id"])
        visuals: list[dict[str, Any]] = []
        for quest in actual:
            visual = copy.deepcopy(quest)
            visual["dependencies"] = [target_to_link.get(str(dep), target_to_link.get(str(dep).lower(), dep)) for dep in quest.get("dependencies", []) or []]
            visuals.append(visual)
        # Only dependencies whose endpoint is represented in this chapter
        # belong on a linked node.  A target's ancestors in another chapter
        # are deliberately omitted instead of becoming fake missing nodes.
        visible_endpoints = actual_ids | set(target_to_link.values())
        for link in links:
            target_entry = global_quests.get(link["linked_quest"]) or folded.get(link["linked_quest"].lower())
            if target_entry is None:
                continue
            target, target_chapter = target_entry
            inherited = [target_to_link.get(str(dep), target_to_link.get(str(dep).lower(), dep))
                         for dep in target.get("dependencies", []) or []]
            inherited = [dep for dep in inherited if dep in visible_endpoints and dep != link["id"]]
            visual = {
                "id": link["id"], "title": target.get("title", target.get("id", "Untitled")),
                "icon": target.get("icon", ""), "x": link["x"], "y": link["y"],
                "size": link["size"], "shape": link["shape"], "optional": target.get("optional", False),
                "dependencies": inherited, "hide_dependency_lines": target.get("hide_dependency_lines"),
                "linked_quest": link["linked_quest"], "linked_chapter": target_chapter.get("filename", target_chapter.get("title", "")),
                "is_quest_link": True, "visual_only": True,
            }
            visuals.append(visual)
        chapter["_render_quests"] = visuals
        chapter["_link_target_map"] = target_to_link
        chapter["quest_link_errors"] = chapter_errors
        errors.extend(chapter_errors)
    return errors


def render_quests(chapter: dict[str, Any]) -> list[dict[str, Any]]:
    """Return visual nodes, falling back safely for an unnormalized chapter."""
    if chapter.get("_render_quests") is not None:
        return chapter["_render_quests"]
    if chapter.get("quest_links"):
        # This supports direct render_chapter/graph_metrics callers while
        # preserving the original chapter.  Cross-chapter targets still need
        # normalize_manifest (or chapters_from_root) so the global map exists.
        return normalize_manifest({"chapters": [chapter]})["chapters"][0].get("_render_quests", [])
    return [normalize_quest(q) for q in chapter.get("quests", []) if isinstance(q, dict)]
def chapters_from_root(root: Path) -> dict[str, Any]:
    ...
    result = normalize_manifest({"architecture": "actual-snbt", "source_root": str(root), "data": data, "chapters": chapters})
    return result
def normalize_manifest(m: dict[str, Any]) -> dict[str, Any]:
    # Keep the input manifest untouched.  This is also what lets callers
    # safely normalize the same source repeatedly while comparing snapshots.
    m = copy.deepcopy(m)
    default_shape = (m.get("data") or {}).get("default_quest_shape", "circle")
    chapters = []
    for ch in m.get("chapters", []) or []:
        if not isinstance(ch, dict):
            continue
        ch.setdefault("default_hide_dependency_lines", False); ch.setdefault("images", [])
        ch["quests"] = [normalize_quest(q) for q in ch.get("quests", []) if isinstance(q, dict)]
        for q in ch["quests"]:
            if not q.get("shape"): q["shape"] = ch.get("default_quest_shape") or default_shape
        ch.setdefault("quest_links", [])
        chapters.append(ch)
    m["chapters"] = chapters
    m["quest_link_errors"] = _link_visuals(chapters)
    return m
...
def shape_points(shape, center, radius):
    if shape in {"circle", "none"}: return None
    if shape == "heart":
        # A low-resolution parametric heart keeps the diagnostic deterministic
        # and gives the native FTB shape a recognisable silhouette.
        out = []
        for i in range(24):
            t = 2 * math.pi * i / 24
            x = 16 * math.sin(t) ** 3
            y = -(13 * math.cos(t) - 5 * math.cos(2 * t) - 2 * math.cos(3 * t) - math.cos(4 * t))
            out.append((center[0] + x * radius / 17, center[1] + y * radius / 17))
        return out
    if shape == "rsquare":
        corner = radius * 0.28
        return [(center[0] - radius + corner, center[1] - radius),
                (center[0] + radius - corner, center[1] - radius),
                (center[0] + radius, center[1] - radius + corner),
                (center[0] + radius, center[1] + radius - corner),
                (center[0] + radius - corner, center[1] + radius),
                (center[0] - radius + corner, center[1] + radius),
                (center[0] - radius, center[1] + radius - corner),
                (center[0] - radius, center[1] - radius + corner)]
    n = {"square":4,"diamond":4,"pentagon":5,"hexagon":6,"octagon":8,"star":10,"gear":16}.get(shape,8); start = -math.pi/4 if shape == "square" else (-math.pi/2 if shape in {"diamond","pentagon","star","gear"} else 0)
    out=[]
    for i in range(n):
...
def _point_on_segment(point, start, end, tolerance=1e-9):
    if abs(orient(start, end, point)) > tolerance:
        return False
    return (min(start[0], end[0]) - tolerance <= point[0] <= max(start[0], end[0]) + tolerance
...
def segment_relation(e1, e2):
    """Classify a pair of non-identical edges, including degenerate layouts."""
    a, b = e1; c, d = e2
    o1, o2, o3, o4 = orient(a, b, c), orient(a, b, d), orient(c, d, a), orient(c, d, b)
    if all(abs(v) <= 1e-9 for v in (o1, o2, o3, o4)):
        # A point contact is a junction, while a positive interval is an
        # accidental overlap that should be reported.
        axis = 0 if abs(a[0] - b[0]) >= abs(a[1] - b[1]) else 1
        first = sorted((a[axis], b[axis])); second = sorted((c[axis], d[axis]))
        overlap = min(first[1], second[1]) - max(first[0], second[0])
        if overlap > 1e-9:
            return "overlap"
        # A shared endpoint is the normal shape of a dependency fork.  It is
        # not an overlap when the two segments only touch at that endpoint.
        if a in (c, d) or b in (c, d):
            return None
        return "touching" if overlap >= -1e-9 else None
    if a in (c, d) or b in (c, d):
        return None  # Edges meeting at their intended endpoint are healthy.
...
def segments_cross(e1, e2):
    """Compatibility wrapper used by callers that only need a boolean."""
    return segment_relation(e1, e2) is not None
...
def graph_metrics(ch):
    qs=render_quests(ch); by={q.get("id"):q for q in qs}; pos={q.get("id"):(float(q.get("x",0)),float(q.get("y",0))) for q in qs}; overlaps=0; spacing=[]
    ...
    # The old strict-orientation crossing test was blind to an edge that ran
    # through another node or to two collinear overlapping dependency lines.
    relations = [segment_relation(a, b) for a, b in itertools.combinations(edges, 2)]
    edge_crossings = sum(relation in {"crossing", "overlap", "touching"} for relation in relations)
    edge_overlaps = sum(relation == "overlap" for relation in relations)
    edge_touches = sum(relation == "touching" for relation in relations)
    edge_through_nodes = 0
    for edge, endpoint_ids in zip(edges, edge_endpoints):
        start, end = edge
        for node_id, node in pos.items():
            if node_id in endpoint_ids:
                continue
            # Quest markers occupy roughly half a layout unit at size 1.0.
            # Check the footprint, rather than only the exact center, so a
            # line grazing an unrelated node is still flagged for review.
            node_radius = max(0.0, float(by[node_id].get("size", 1.0))) * 0.5
            if _distance_to_segment(node, start, end) <= node_radius + 1e-9 and node not in (start, end):
                edge_through_nodes += 1
    ...
    links = [q for q in qs if q.get("is_quest_link")]
    return {"node_overlaps":overlaps,"estimated_label_collisions":estimated_label_collisions(ch),
            "visible_edge_crossings":edge_crossings,"visible_edge_count":len(edges),
            "visible_edge_overlaps":edge_overlaps,"visible_edge_touching_intersections":edge_touches,
            "visible_edge_through_unrelated_nodes":edge_through_nodes,
            "edge_overlaps":edge_overlaps,"edge_through_unrelated_nodes":edge_through_nodes,
            "linked_visual_nodes":len(links),"quest_link_errors":list(ch.get("quest_link_errors", [])),
            "hidden_dependency_edges":hidden,"missing_dependency_nodes":missing,
            "edge_lengths":{"count":len(lens),"mean":round(sum(lens)/len(lens),3) if lens else 0,"max":round(max(lens,default=0),3)},"node_spacing":{"mean":round(sum(spacing)/len(spacing),3) if spacing else 0,"min":round(min(spacing),3) if spacing else 0},"cluster_separation":cluster,"spine_alignment":{"core_x":core_spine,"core_rms":core_align,"all_nodes_rms":round(align,3)},"spine_alignment_rms":round(align,3),"disconnected_components":comps}
...
def render_chapter(ch,images,models,unresolved,unresolved_icons,out,index,detail=False,orthogonal=False):
    qs=render_quests(ch); W,H,scale,pt=chapter_viewport(ch, detail=detail)
    ...
    collisions=0
    for q in qs:
        center=pos[q["id"]]; radius=29.*float(q.get("size",1.)); shape=clean(q.get("shape")) or clean(ch.get("default_quest_shape")) or "circle"; shape=shape if shape in SHAPES else "circle"; fill=tuple(min(255,int(c*1.08)) for c in base); outline=(91,211,224) if q.get("is_quest_link") else ((247,220,125) if not q.get("optional") else (170,185,200)); points=shape_points(shape,center,radius)
        ...
        if q.get("is_quest_link"):
            # Cyan accent plus explicit metadata makes the node identifiable
            # as a chapter link without presenting copied tasks or rewards.
            draw.ellipse((center[0]-radius*.42, center[1]-radius*.42,
                          center[0]+radius*.42, center[1]+radius*.42),
                         outline=(91,211,224), width=2 if not detail else 3)
...
def dense_crops(ch,overview,out,index,orthogonal=False):
    qs=render_quests(ch); dense=[q for a,b in itertools.combinations(qs,2) if math.dist((float(a.get("x",0)),float(a.get("y",0))),(float(b.get("x",0)),float(b.get("y",0))))<4 for q in (a,b)]
    if not dense: return []
...
def main(argv=None):
    ...
    link_warnings=[error for ch in current.get("chapters",[]) for error in ch.get("quest_link_errors", [])]
    unresolved_icon_list=sorted(unresolved_icons)
    warnings=[f"background art unresolved: {key}" for key in unresolved_list]+shape_warnings+link_warnings+[f"quest icon unresolved: {key}" for key in unresolved_icon_list]
